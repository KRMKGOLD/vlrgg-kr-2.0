...
observability custom role의 정확한 22개 권한은 `serviceusage.services.use`, `resourcemanager.projects.get`, `resourcemanager.projects.getIamPolicy`, `logging.logEntries.list`, `errorreporting.groups.list`, `errorreporting.errorEvents.list`, `errorreporting.groupMetadata.get`, `errorreporting.groupMetadata.update`, `errorreporting.applications.list`, `monitoring.metricDescriptors.get`, `monitoring.timeSeries.list`, `monitoring.notificationChannels.get`, `monitoring.alerts.list`, `monitoring.alertPolicies.get`, `monitoring.alertPolicies.list`, `monitoring.uptimeCheckConfigs.get`, `monitoring.uptimeCheckConfigs.list`, `monitoring.alertPolicies.create`, `monitoring.alertPolicies.update`, `monitoring.alertPolicies.delete`, `monitoring.uptimeCheckConfigs.create`, `monitoring.uptimeCheckConfigs.delete`다. Cloud Run 권한은 private validation service에 한정한 기존 `roles/run.admin`과 ID-token 발급 binding을 재사용한다. channel 생성 권한, `iam.serviceAccounts.get`, project-wide public IAM 변경은 추가하지 않는다. 검증된 Monitoring principal에 대한 validation service `roles/run.invoker`만 journal과 함께 추가·복원한다.

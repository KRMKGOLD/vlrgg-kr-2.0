...
`observability-validate`는 cloud 인증 직후 Monitoring incident 조회·Error Reporting 그룹 조회·알림 채널 조회를 먼저 확인한다. 이 읽기 전용 확인이 실패하면 recovery journal·검증 image·revision·트래픽을 변경하지 않는다. 검증 revision 배포 후에도 같은 확인을 반복한다. 실패 로그에는 고정된 API 분류와 HTTP 상태·curl 종료 코드만 남기며 URL·응답 본문·인증 정보는 출력하지 않는다.

복원 journal의 `baselineRevision`은 immutable revision 조회를 위한 전체 리소스 경로로 보관한다. Cloud Run v2 `traffic[].revision` 요청과 read-back 비교에는 마지막 경로 요소인 짧은 revision 이름을 사용한다. 복원 순서는 정상 revision으로 트래픽 전환 → template 복원 → private health/query 확인 → 소유 자원 정리 → journal 삭제를 유지한다.

observability custom role의 정확한 23개 권한은 `serviceusage.services.use`, `resourcemanager.projects.get`, `resourcemanager.projects.getIamPolicy`, `logging.logEntries.list`, `logging.notificationRules.create`, `errorreporting.groups.list`, `errorreporting.errorEvents.list`, `errorreporting.groupMetadata.get`, `errorreporting.groupMetadata.update`, `errorreporting.applications.list`, `monitoring.metricDescriptors.get`, `monitoring.timeSeries.list`, `monitoring.notificationChannels.get`, `monitoring.alerts.list`, `monitoring.alertPolicies.get`, `monitoring.alertPolicies.list`, `monitoring.uptimeCheckConfigs.get`, `monitoring.uptimeCheckConfigs.list`, `monitoring.alertPolicies.create`, `monitoring.alertPolicies.update`, `monitoring.alertPolicies.delete`, `monitoring.uptimeCheckConfigs.create`, `monitoring.uptimeCheckConfigs.delete`다. [로그 기반 알림 생성](https://docs.cloud.google.com/logging/docs/alerting/log-based-alerts)에 필요한 내부 notification rule 권한도 포함한다. Cloud Run 권한은 private validation service에 한정한 기존 `roles/run.admin`과 ID-token 발급 binding을 재사용한다. channel 생성 권한, `iam.serviceAccounts.get`, project-wide public IAM 변경은 추가하지 않는다. 검증된 Monitoring principal에 대한 validation service `roles/run.invoker`만 journal과 함께 추가·복원한다. Artifact Registry의 기존 Writer는 삭제 권한이 없으므로, 검증 image 정리용 custom role에는 `artifactregistry.tags.delete`와 `artifactregistry.versions.delete`만 두고 기존 registry repository에 한정해 workflow 계정에 부여한다. `gcloud artifacts docker images delete --delete-tags`는 태그 삭제 후 digest version을 삭제하며, cleanup helper의 run-owned digest·tag·사용 revision 검사를 그대로 적용한다. 프로젝트 전체 Repository Admin은 부여하지 않는다.

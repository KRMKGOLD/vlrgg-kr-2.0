...
`.github/workflows/deploy-server.yml`의 `operation`은 기본 `deploy`, `observability-validate`, `observability-restore`만 허용한다. 기존 workflow concurrency와 `cancel-in-progress: false`, exact SHA CI, protected environment, WIF를 재사용한다. validation은 production token·traffic·policy를 건드리지 않고, restore는 source/test/image build와 push를 건너뛴다. workflow 밖의 수동 cloud 변경은 validation 시간 동안 금지한다.

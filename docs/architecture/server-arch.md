...
- 실패 진단에는 제한된 canonical origin과 안전한 cause 분류만 사용한다. raw request path/query/IP/header와 예외 메시지는 넣지 않는다. `INTERNAL_ERROR`와 `SOURCE_PARSING_FAILURE`만 Error Reporting용 ERROR 이벤트이며, runtime에서 확인한 서버·라이브러리 class/member만 안전한 Java stack 문법으로 재구성한다. filename은 runtime class의 top-level 이름에서 nested suffix와 Kotlin `Kt` suffix를 제거해 파생한 `<Name>.kt` 또는 `<Name>.java`와 일치할 때만 채택한다. line은 양수 JVM `Int`일 때만 filename 뒤에 출력하며, 이 검사는 실제 source/bytecode 위치의 진위를 증명하지 않는다. 임의 stack 문자열, suppressed 예외, Throwable serialization은 금지한다.
- 안전한 위치는 전체 cause 합계 frame 32개, root 뒤 cause link 3개, 후보 검사 128개로 제한한다. class/member가 유효하지 않거나 runtime에서 확인되지 않은 후보는 frame 전체를 drop하고 `truncation.frames`를 표시한다. class/member는 유효하지만 filename이 파생 규칙과 다르면 `Unknown Source`, 유효한 filename의 nonpositive line은 file-only, native frame은 `Native Method`로 출력하며 이 위치 표기 downgrade만으로 누락 flag를 세우지는 않는다. 최종 stdout record는 줄바꿈까지 UTF-8 16 KiB 이하여야 하며 바이트 중간을 자르지 않는다.
...
- trace는 애플리케이션에 실제로 전달된 유효한 단일 `X-Cloud-Trace-Context`에서만 가져온다. 신뢰된 project 환경과 header 전체를 검증한 뒤 `logging.googleapis.com/trace`만 출력하며, 원본 header, `spanId`, `trace_sampled`는 기록하지 않는다. invalid·중복·과도한 header는 trace 파생 필드 전체를 생략한다.
- Cloud Run 실환경에서는 관리형 frontend가 trace context를 제공할 수 있으므로, 클라이언트의 원래 header가 없거나 잘못됐다는 이유만으로 로그의 trace 부재를 요구하지 않는다. 실환경 검증은 지정한 유효 trace의 보존과 실제 요청 로그 연결을 확인하고, 잘못된 header 자체의 미승격은 로컬 formatter 테스트로 검증한다.

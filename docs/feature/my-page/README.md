...
Match 알림을 설정하면 Match 즐겨찾기와 current anonymous Target의 서버 notification subscription이 함께 생성된다. MyPage에서 Match 즐겨찾기를 제거하면 해당 subscription도 함께 취소된다. 이 App 흐름과 실제 Firebase SDK 연동은 Stage 2다.
...
- Match를 제거하면 current Target의 해당 서버 notification subscription 취소를 먼저 요청하고, 로컬 즐겨찾기는 제거 대기 상태로 유지한다. 서버가 alarm OFF를 확인한 뒤에만 로컬 삭제를 확정한다.
...
- [ ] Match 즐겨찾기 제거는 current Target의 대응 서버 subscription을 먼저 취소하고 alarm OFF 확인 뒤에만 로컬 삭제를 확정하며 반복 요청은 alarm OFF로 수렴한다.

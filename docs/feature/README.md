...
아래 상태는 현재 `main`의 구현 범위를 기록한 것이며, 1차 MVP의 제품 범위나 수용 기준을 변경하지 않는다. `app/shared`는 아직 템플릿 단계이므로, 앱 화면·원격 DTO/Domain 매핑·내비게이션·로컬 즐겨찾기 구현은 이 표의 모든 feature에서 미구현이다.
